...
uint8_t mode = 1;
// Bi?n global
uint16_t desired_setpoint = 2000;  // Giá tr? m?c d?nh
uint8_t setpoint_changed = 0;      // C? báo thay d?i

// Buffer nh?n UART
volatile char uart_rx_buffer[10];
volatile uint8_t uart_rx_index = 0;
volatile uint8_t uart_rx_complete = 0;

// Hàm c?p nh?t setpoint
void UpdateSetpoint(uint16_t new_setpoint)
{
        desired_setpoint = new_setpoint;
        light_tanh.setpoint = (float)new_setpoint;
        light_tanh.integral = 0.0f;  // Reset integral khi d?i setpoint
        setpoint_changed = 1;
...
GPIOA->BRR = 1 << 5;
	uint32_t last = 0;
	LightControl_Init(desired_setpoint);
...
//		
		// X? lý setpoint m?i t? UART
        if (uart_rx_complete) {
            uint16_t new_sp = atoi(uart_rx_buffer);  // Chuy?n chu?i ? s?
                UpdateSetpoint(new_sp);       
            uart_rx_complete = 0;
        }
		if(uwTick - last >= 2000){
		DHT22_Start();
...
if (mode == 0){
	TIM2->CCR2 = (received - '0')*100;
	return;
}
// Nh?n chu?i s? (setpoint)
